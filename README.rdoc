...
developing them. WxIRB puts you "inside" a Wx::App.run event loop, letting
you do things with window objects much more easily.
...
*   '$wxirb.set_binding(bind)' sets the object binding to 'bind' for use in
     MimickIRB's eval() 'runner'. See 'Object Binding' for more information.
...
* As mentioned above, WxIRB::BaseFrame also has an instance method called
  ...
  Here's a short example from WxIRB running 'wxirb.rb' directly:
